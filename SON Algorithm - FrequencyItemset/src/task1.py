...
def create_L1_phase1(basket_partition,support)->list:
    ...
    # loop through basket to find count of singletons
...
if __name__=='__main__':
    ...
    header=rdd.first()
     ...
     # check case num
    if case_num==1:
        basket=rdd.filter(lambda r:r!=header).\
        ...
        basket=rdd.filter(lambda r:r!=header).\
                   map(lambda r:[r.split(',')[1],r.split(',')[0]]).\
                   groupByKey().\
                   map(lambda r:list(set(r[1])))
        basket_length=basket.count()
    ...
    # write output
    write_output(format_list(phase_1),format_list(freq_sets),output_file)
