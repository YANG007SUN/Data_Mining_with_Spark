...
if __name__=='__main__':
    ...
    final_output=[row+[p] for p, row in zip(list(predictions),output_testing)]

    # get pred value and true value
    truth,pred=[],[]
    for row in final_output:
        truth.append(float(true_dict[(row[0],row[1])]))
        pred.append(row[2])
    ...
    # output
    output_file(final_output,output_path)

    end_time=time()
    print(f'Total time is {end_time-start_time}, RMSE:{rmse}')
